# Functions as in untils.py init_params

library(CVXR) # for init_beta_b_convex_QP()
library(Matrix) # for statdist
...
init_params <- function(X, rankings,  mat_Pij, method_beta_b_init = "QP"){
    # n: number of items
    ...
    if (method_beta_b_init == 'LS'){
        beta_b <- init_beta_b_ls(X = X, rankings = rankings)
    } else {
        beta_b <- init_beta_b_convex_QP(X = X, rankings = rankings, mat_Pij = mat_Pij)
    }
    ...
    exp_beta <- init_exp_beta(X, rankings, mat_Pij)
...
init_beta_b_ls <- function(X, rankings){
    ...
    sum_auto_corr <- 0
    m <- nrow(rankings)
    n <- ncol(rankings) # assume rankings of same length
    for (r in seq_len(m)){
        for (i in seq_len(n)){
            winner <- rankings[r, i]
...
init_beta_b_convex_QP <- function(X, rankings, mat_Pij){
    # n: number of items
    # p: number of features
    # :param rankings: (c_l, A_l): 1...M
    # :param X: n*p, feature matrix
...
est_sum_dij_dijT <- function(X, rankings, mat_Pij = NULL){
    # n: number of items
    # p: number of features
    # :param rankings: (c_l, A_l): 1...M
    # :param X: n*p, feature matrix
...
init_exp_beta <- function(X, rankings, mat_Pij = NULL){
...
statdist <- function(generator, method = "power", v_init = NULL){
...
objective <- function(weights, rankings){
    ll <- 0
    # against log(0), add epsilon
    for (r in seq_len(nrow(rankings))){
        sum_weights <- sum(weights[rankings[r,]])
        winner <- rankings[r, 1]
